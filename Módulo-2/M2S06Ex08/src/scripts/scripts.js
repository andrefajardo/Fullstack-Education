...
export function f_page1(){
    ...
    imprimeNomeAluno.innerText = alunoObj.name;
    imprimeidadeAluno.innerText = alunoObj.age;
    imprimeSerieAluno.innerText = alunoObj.grade;
    imprimeNomeEscola.innerText = alunoObj.school;
    imprimeMateriaFavorita.innerText = alunoObj.subject;
...
export function f_preparaTela(e) {
  e.preventDefault();
  window.location.href = "http://127.0.0.1:5501/pages/page01.html";
}
...
function f_inserirDados() {
  ...
  calcularMediaGeral()
  const materiasNotasJSON = JSON.stringify(materiaNotas);
  localStorage.setItem('notas', materiasNotasJSON);
...
export async function f_listarAlunos() {

  const listaAlunos = document.getElementById('lista-alunos');
  debugger
  
    fetch(`../../assets/PseudoServer/alunos.json`)
    .then((response)=>{
        return response.json();
    })
    .then((data)=>{
